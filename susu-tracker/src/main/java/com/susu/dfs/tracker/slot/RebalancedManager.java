...
import com.susu.dfs.tracker.task.TrackerThreadFactory;
 ...
 */
@Slf4j
public class RebalancedManager {

    private TrackerSlotLocal slotLocal;
    private LinkedBlockingQueue<RebalancedSlotInfo> requestQueue = new LinkedBlockingQueue<>(100);
    private RebalancedSlotInfo rebalancedSlotInfo = null;


    public RebalancedManager(TrackerSlotLocal slotLocal) {
        this.slotLocal = slotLocal;
        TrackerThreadFactory threadFactory = new TrackerThreadFactory("RebalancedManager-");
     ...
     * 添加一个重平衡任务
     *
     * @param rebalancedSlotInfo 重平衡任务
     * @throws InterruptedException 异常
     */
    public void add(RebalancedSlotInfo rebalancedSlotInfo) throws InterruptedException {
        requestQueue.put(rebalancedSlotInfo);
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if (rebalancedSlotInfo != null && rebalancedSlotInfo.getTrackerIdList().contains(slotInfo.getApplyTrackerId())) {
                        log.warn("上次重平衡的时候，已经把该节点考虑进去了，不需要再处理该节点的重平衡请求：[needRebalanceNodeId={}]",
                                slotInfo.getApplyTrackerId());
                        continue;
                    }
                    rebalancedSlotInfo = slotInfo;
                    log.info("开始执行节点的重平衡请求：[nodeId={}]", rebalancedSlotInfo.getApplyTrackerId());
                    Map<Integer, Set<Integer>> nodeSlotsMapSnapshot = rebalancedSlotInfo.getTrackerOfSlotsSnapshot();
                    Map<Integer, Integer> slotNodeMapSnapshot = rebalancedSlotInfo.getSlotsOfTrackerSnapshot();
                    List<Integer> nodeIdList = rebalancedSlotInfo.getTrackerIdList();
                    Set<Integer> rebalancedNodes = rebalance(slotNodeMapSnapshot, nodeSlotsMapSnapshot, nodeIdList);
                    rebalancedSlotInfo.setTrackerIdSet(rebalancedNodes);
                    slotLocal.setRebalancedInfo(rebalancedSlotInfo);
                    rebalancedSlotInfo.waitRemoveMetadataCompleted(rebalancedNodes);
                    log.info("完成了一轮重平衡：[needRebalanceId={}]", rebalancedSlotInfo.getApplyTrackerId());
                } catch (Exception e) {
